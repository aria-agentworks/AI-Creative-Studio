...
const MODELS = [
  { id: 'black-forest-labs/flux-2-pro', name: 'Flux 2 Pro', desc: 'Best quality', tier: 'premium' },
  { id: 'black-forest-labs/flux-2-dev', name: 'Flux 2 Dev', desc: 'High quality, balanced', tier: 'standard' },
  { id: 'black-forest-labs/flux-2-flex', name: 'Flux 2 Flex', desc: 'Fast, flexible', tier: 'standard' },
  { id: 'black-forest-labs/flux-2-klein', name: 'Flux 2 Klein', desc: 'Ultra fast, compact', tier: 'free' },
];
...
export default function ImageGenerator({ apiKey }) {
  ...
  const [selectedAr, setSelectedAr] = useState(ASPECT_RATIOS[0]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    const size = `${selectedAr.w}x${selectedAr.h}`;
    ...
    try {
      ...
      if (data.images && data.images.length > 0) {
        const newImages = data.images.map((img, i) => ({
          ...
          aspectRatio: selectedAr.label,
          seed: data.seeds ? data.seeds[i] : seed,
  ...
  }, [prompt, selectedModel, selectedAr, selectedStyle, seed, numImages, isGenerating]);
    ...
    <div className="flex-1 flex flex-col overflow-hidden relative">
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-8 md:py-12">
          ...
          <div className="text-center mb-8 animate-fade-in-up">
            ...
            <p className="text-white/40 text-sm font-medium tracking-wide">
              Powered by NVIDIA NIM — Flux 2 Models
            </p>
          </div>
                ...
                <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
                        ...
                        {MODELS.map((model) => (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold ${
                              ...
                              {model.name.includes('Pro') ? 'P' : model.name.includes('Dev') ? 'D' : model.name.includes('Flex') ? 'F' : 'K'}
                            </div>
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <span className="text-xs font-bold text-white">{selectedAr.label}</span>
                    </button>
                  </div>
            ...
            <div className="mt-4 animate-fade-in-up">
              <div className="bg-[#111]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 flex flex-col gap-5">
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {ASPECT_RATIOS.map((ar) => (
                      ...
                      >
                        {ar.label}
                        <span className="text-[9px] ml-1 opacity-50">{ar.w}x{ar.h}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-4 flex-wrap">
                  <div className="flex-1 min-w-[150px]">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        ...
                        className="text-[11px] font-bold text-[#d9ff00] hover:text-[#e5ff33] transition-colors cursor-pointer"
                      >
                        Randomize
                      </button>
                    </div>
                  ...
                  <div className="flex-1 min-w-[150px]">
                    <div className="flex items-center justify-between mb-2">
                      <label className="text-xs font-bold text-white/40 uppercase tracking-wider">Number of Images</label>
                      <span className="text-xs font-bold text-[#d9ff00]">{numImages}</span>
                    </div>
                    <input
                      type="range"
                      min="1"
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar px-4 md:px-6 pb-6">
                  ...
                  <div className="image-overlay">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-white/40">{image.aspectRatio}</span>
                      </div>
          ...
          <div className="max-w-6xl mx-auto mt-4">
            <div className="image-grid">
              {Array.from({ length: numImages }).map((_, i) => (
                <div key={i} className="aspect-square rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-center">
        ...
        {generatedImages.length === 0 && !isGenerating && (
          <div className="max-w-4xl mx-auto mt-16 text-center">
            ...
            <p className="text-white/10 text-xs mt-2">Enter a prompt above and click Generate to create</p>
          </div>
        )}
      </div>
            ...
            <div className="mt-3 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-xs font-bold text-white transition-all cursor-pointer"
                >
                  Download
                </button>
              </div>
      ...
      {(showModelDropdown || showAdvanced) && (
        <div
          ...
          onClick={() => { setShowModelDropdown(false); setShowAdvanced(false); }}
        />
      )}
    </div>
  );
